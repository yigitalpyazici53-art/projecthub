    ...
    <main style={pageStyle}>
      ...
      <div className="page-z" style={{ maxWidth: 900, margin: "0 auto", paddingTop: 80, paddingBottom: 80 }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 20, marginTop: 16, flexWrap: "wrap" }}>
            {[
              ...
              <div key={label} style={pointPillStyle}>
                <span style={{ color: "var(--text-primary)", fontWeight: 700 }}>{pts}</span>
        ...
        <div className="animate-fade-up animate-delay-1" style={cardStyle}>
          ...
          {loading ? (
            <div style={{ padding: "40px 0", textAlign: "center", color: "var(--text-muted)", fontSize: 14 }}>Loading leaderboard…</div>
          ) : builders.length === 0 ? (
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      gap: 16,
                      padding: "16px 0",
                      borderBottom: idx < builders.length - 1 ? "1px solid var(--border-subtle)" : "none",
                    }}
                  >
                    {/* Rank */}
                    <div style={{
                    ...
                    <div style={{
                      ...
                      overflow: "hidden",
                      border: isTop3 ? `2px solid ${rank === 1 ? "var(--tint-amber-text)" : rank === 2 ? "var(--text-muted)" : "#cd7c2d"}` : "2px solid transparent",
                    }}>
                      {builder.avatar_url
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3, flexWrap: "wrap" }}>
                        <Link
                          ...
                          style={{ fontSize: 15, fontWeight: 700, color: "var(--text-primary)", fontFamily: "var(--font-sans)", textDecoration: "none" }}
                        >
                          {builder.full_name || builder.username || "Builder"}
                        </Link>
                        {isRising && (
                          <span style={{ fontSize: 10, fontWeight: 700, color: "var(--tint-amber-text)", background: "var(--tint-amber-bg)", border: "1px solid var(--tint-amber-bg)", borderRadius: 10, padding: "2px 7px" }}>
                            ↑ Rising
                          </span>
                        )}
                      </div>
                      {builder.university && (
                        <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 6 }}>{builder.university}</div>
                      )}
                      {/* Points bar */}
                      <div style={{ height: 4, borderRadius: 2, background: "var(--surface-raised)", overflow: "hidden", maxWidth: 300 }}>
                        <div style={{
                          height: "100%",
                          width: `${barPct}%`,
                          borderRadius: 2,
                          background: rank === 1 ? "linear-gradient(90deg, #F59E0B, #FBBF24)"
                            : rank === 2 ? "linear-gradient(90deg, #CBD5E1, #E2E8F0)"
                            : rank === 3 ? "linear-gradient(90deg, #D97706, #F59E0B)"
                    ...
                    <div style={{ display: "flex", gap: 16, flexShrink: 0, alignItems: "center" }}>
                      <div className="leaderboard-stat-col" style={{ textAlign: "center", display: "flex", flexDirection: "column", gap: 2 }}>
                        ...
                        <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-secondary)" }}>{builder.projectCount}</span>
                      </div>
                      <div className="leaderboard-stat-col" style={{ textAlign: "center", display: "flex", flexDirection: "column", gap: 2 }}>
                        <span style={{ fontSize: 11, color: "var(--text-muted)" }}>Connects</span>
                        <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-secondary)" }}>{builder.connectionCount}</span>
                      </div>
                      <div className="leaderboard-stat-col" style={{ textAlign: "center", display: "flex", flexDirection: "column", gap: 2 }}>
                        <span style={{ fontSize: 11, color: "var(--text-muted)" }}>Endorsements</span>
                        <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-secondary)" }}>{builder.endorsementCount}</span>
                      </div>
                      {/* Points badge */}
                      <div style={{
                        padding: "6px 14px",
                        borderRadius: 20,
                        background: isTop3 ? (rank === 1 ? "var(--tint-amber-bg)" : rank === 2 ? "rgba(156,163,175,0.1)" : "rgba(205,124,45,0.1)") : "#F5F5F3",
                        border: `1px solid ${isTop3 ? (rank === 1 ? "var(--tint-amber-bg)" : rank === 2 ? "rgba(156,163,175,0.25)" : "rgba(205,124,45,0.25)") : "#E8E8E4"}`,
                        minWidth: 64,
                        textAlign: "center",
                      }}>
                        <div style={{
                          ...
                          fontWeight: 800,
                          fontFamily: "var(--font-sans)",
                          color: isTop3 ? (rank === 1 ? "var(--tint-amber-text)" : rank === 2 ? "var(--text-muted)" : "var(--tint-amber-text)") : "var(--text-secondary)",
                        }}>
                          {builder.points}
                        </div>
                        <div style={{ fontSize: 9, color: "var(--text-muted)", fontWeight: 600 }}>PTS</div>
                      </div>
                    </div>
                  </div>
                );
        ...
        <div className="animate-fade-up animate-delay-2" style={{ ...cardStyle, marginTop: 20 }}>
          ...
          {loading ? (
            <div style={{ padding: "24px 0", textAlign: "center", color: "var(--text-muted)", fontSize: 14 }}>Loading…</div>
          ) : universities.length === 0 ? (
            <p style={{ color: "var(--text-muted)", fontSize: 14 }}>No university data yet.</p>
          ) : (
              ...
              {universities.map((uni, idx) => {
                ...
                return (
                  <div key={uni.name} style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    {/* Rank + flag */}
                    <div style={{ width: 28, textAlign: "center", flexShrink: 0, fontSize: 13, fontWeight: 700, color: "var(--text-muted)" }}>
                      {idx + 1}
                    </div>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                        ...
                        <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text-secondary)", flexShrink: 0, marginLeft: 12 }}>
